...
export default {
  ...
  mounted() {
    this.userStore.getFromURLWithAuth({url:"users/" + this.userStore.user.user.id}).then(rsp => {
        this.user = {...rsp}
    })
  },
  watch: {},
...
<template>
  <v-card class="ma-3"> 
    <v-sheet>
        ...
        <v-sheet v-if="userStore.user.user.profile.automatically_verifiable && !userStore.user.user.profile.verified">
            Dein Account ist nicht automatisch verifizierbar. Bitte bringe eine Bluecard und eine Studienbescheinigung zu deiner nächsten Ausleihe mit.
        </v-sheet>
    </v-sheet>
  </v-card>
</template>
